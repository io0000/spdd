...
namespace Microsoft.Xna.Framework.Audio
{
    internal static class AudioLoader
    {
        ...
        public static ALFormat GetSoundFormat(int format, int channels, int bits)
        {
            switch (format)
            {
                ...
                case FormatMsAdpcm:
                    ...
                    switch (channels)
                    {
                        case 1: return (ALFormat)4866; //ALFormat.MonoMSAdpcm;
                        case 2: return (ALFormat)4867; //ALFormat.StereoMSAdpcm;
                        default: throw new NotSupportedException("The specified channel count is not supported.");
                    }
                case FormatIeee:
                    // IEEE Float
                    switch (channels)
                    {
                        case 1: return ALFormat.MonoFloat32Ext;    //ALFormat.MonoFloat32;
                        case 2: return ALFormat.StereoFloat32Ext;  //ALFormat.StereoFloat32;
                        default: throw new NotSupportedException("The specified channel count is not supported.");
                    }
                case FormatIma4:
                    // IMA4 ADPCM
                    switch (channels)
                    {
                        case 1: return ALFormat.MonoIma4Ext;    //ALFormat.MonoIma4;
                        case 2: return ALFormat.StereoIma4Ext;  //ALFormat.StereoIma4;
                        default: throw new NotSupportedException("The specified channel count is not supported.");
                    }
                default:
        ...
        public static int SampleAlignment(ALFormat format, int blockAlignment)
        {
            ...
            switch (formatValue)
            {
                case 4864: // ALFormat.MonoIma4:
                    return (blockAlignment - 4) / 4 * 8 + 1;
                case 4865: // ALFormat.StereoIma4:
                    return (blockAlignment / 2 - 4) / 4 * 8 + 1;
                case 4866: // ALFormat.MonoMSAdpcm:
                    return (blockAlignment - 7) * 2 + 2;
                case 4867: //ALFormat.StereoMSAdpcm:
